...
export const SideButtons = () => {
    ...
    const getTotalUnreadCount=async()=>{
        try{
          ...
          const res=  await chatServ.getTotalUnreadCount(obj)
            if(res){
                setPersonalCount(res.unreadCountPersonal)
                setGroupCount(res.unreadCountGroup)
            }
        }catch(err){
            console.log(err)
        }
    ...
    return (
        <div className="message-buttonWrap" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div className="message-buttonWrapper"
                onClick={() => {
                    if (!isGroupChat) {
                        setisLoading(true)
                        setShowChat(true)
                        ...
                        setMessageBoxState([])
                        localStorage.setItem("isgroupchat", JSON.stringify(true))
                        setisGroupChat(true)
            ...
            >
                <span className="message-count">
                    <h className="message-countheight">{groupCount}</h>
                </span>
            ...
            <div className="message-buttonWrapper "
                onClick={() => {
                    if (isGroupChat) {
                        ...
                        localStorage.setItem("messageboxstate", JSON.stringify([]))
                        setMessageBoxState([])
                        localStorage.setItem("isgroupchat", JSON.stringify(false))
            ...
            >
                <span className="message-count">
                    <h className="message-countheight">{personalcount}</h>
                </span>
                <div className="message-button">
                    <svg
                        width="30"
